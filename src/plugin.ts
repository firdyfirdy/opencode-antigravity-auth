      ...
      loader: async (getAuth: GetAuth, provider: Provider): Promise<LoaderResult | Record<string, unknown>> => {
        ...
        return {
          ...
          async fetch(input, init) {
            ...
            // Track which account we've already shown a toast for in THIS specific request
            // Prevents spamming the same toast during retries, but allows it on account switch
            let currentRequestShownAccountIndex = -1;
            ...
            while (true) {
              ...
              // Show toast when switching to a different account OR if this is the first time for this request
              // We track currentRequestShownAccountIndex to avoid spamming toast during retries for the SAME account
              if (account.index !== currentRequestShownAccountIndex) {
                ...
                // Mark as shown for this request cycle
                currentRequestShownAccountIndex = account.index;
                // Also update global manager state (though we bypass its check now, good to keep it in sync)
